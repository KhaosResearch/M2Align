          ...
          if (line.substring(0, 4).equals("ATOM")) {
            ...
            if (!last_chain.equals(chain_ID)) {
              ...
              if (!_pdb_chains.containsKey(last_chain)) {
                System.out.println("Warning: Chain " + last_chain
                    + " does not have an SEQRES field and is ignored");
                while ((line = reader.readLine()) != null) {
            ...
            chain_it._atom_list.get(res_ID - 1).put(atom_name,
                new Atom(chain_ID, res_ID, amino_accid.charAt(0), atom_name, atom_serial_number, x,
                    y, z));
          }
        }
