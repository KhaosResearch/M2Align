...
import org.uma.khaos.m2align.solution.util.ArrayChar;
import org.uma.khaos.m2align.util.MSADistance;
    ...
    for (int l = 0; l < lengthSequences; l++) {
      ...
      for (i = 0; i < numberOfVariables - 1; i++) {
        for (j = i + 1; j < numberOfVariables; j++) {
          sumOfPairsOfLthColumn += weightMatrix[i][j] * sustitutionMatrix
              .getDistance(decodedSequences[i][l], decodedSequences[j][l]);
        }
      }
  ...
  public double[][] getWMatrix(List<ArrayChar> Seqs) {
    ...
    for (int i = 0; i < NumSeqs - 1; ++i) {
      ...
      for (int j = i + 1; j < NumSeqs; ++j) {
        ...
        wMatrix[i][j] =
            1 - (distanceMatrix.getLevenshteinDistance(Si, Sj) / (Si.length > Sj.length ? Si.length
                : Sj.length));
      }
    }
