...
public class ShiftClosedGapsMSAMutation implements MutationOperator<MSASolution> {
  ...
  @Override public MSASolution execute(MSASolution solution) {
    if (null == solution) {
  ...
  public void doMutation(MSASolution solution) {
    if (randomGenerator.nextDouble() < mutationProbability) {
      ...
      start=gapsGroup.get(posClosedGaps); numberofgaps=gapsGroup.get(posClosedGaps+1)-start+1;
      //System.out.println("start " + start + " Number of Gaps " + numberofgaps );
      gapsGroup.remove(posClosedGaps); gapsGroup.remove(posClosedGaps);
  ...
  public Integer getClosedGaps(List<Integer> gapsGroup){
    ...
    //}while (gapsGroup.get(posClosedGaps+1)-gapsGroup.get(posClosedGaps)<1);
    
    //System.out.println("Pos " + posClosedGaps.get(0) + " " + posClosedGaps.get(1) + " Gaps = " + sequence.subSequence(posClosedGaps.get(0), posClosedGaps.get(1)+1));
